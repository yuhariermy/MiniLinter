...
let pAequorFactory = (specimenNum, dna) => {
  return {
    ...
    mutate() {
      ...
      let index = Math.floor(Math.random() * this.dna.length); // random index of the base to change
      ...
      this.dna.splice(index, 1, random); // replacing the base with a random one
